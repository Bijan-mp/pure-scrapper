...
def get_soup(url=None, html_doc=None, parser="html.parser"):
    ...
    if url is not None:
        ...
        # To prevent error : "UnicodeEncodeError: 'charmap' codec can't encode character '\u011f'"
        # use below command
...
# Get main page
url = BASE_URL + MOBILE_CATEGORY_URL

soup = get_soup("https://www.digikala.com/search/category-mobile-phone/?sortby=20").find_all(
    "div",
    attrs={
        "class": [
            "c-product-box",
...
for element in soup:
    try:
        ...
        product_short_url = BASE_URL + re.search("/product/dkp-[\d]*/", product_url)[0]
        ...
        element_data_object["short_url"] =  product_short_url
        element_data_object["url"] = BASE_URL + product_url
        element_data_object["img_url"] = product_img_url
        if price_element != None and price_element.find("del"):
            element_data_object["price_before_discount"] = price_element.find(
                "del"
            ).contents[0]
            print(element_data_object["price_before_discount"])
        # product_list.append(element_data_object)
        c += 1

        print("id :", element_data_object["id"], " c=: ", c)
    except Exception:
...
#     # Fetch Product price list
#     url = BASE_URL + PRODUCT_PRICE_HISTORY_BASE_URL + str(product_id) + "/"
#     print(url)
...
def fetch_product_rating(product_id):
    ...
    for element in rating_elements:
        ...
        rating = element.find("div", attrs={"class": "c-rating__rate js-rating-value"})[
            "data-rate-value"
        ]
